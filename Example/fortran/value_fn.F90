...
SUBROUTINE minv(a11,a12, a,b,c, an1,an2, ns, Rv, f) 

INTEGER(4), INTENT(in) :: ns
REAL(dp), INTENT(in):: a, b, c, a11, a12
REAL(dp), INTENT(in):: an1,an2 !these are the parameters in last row of matrix A
REAL(dp), INTENT(in), DIMENSION(ns-1)::Rv
REAL(dp), INTENT(out), DIMENSION(ns-1) :: f
INTEGER :: j, j1
REAL(dp), DIMENSION(ns-1) :: u, y, l
!==============================

  !Decomposing Am into lower and upper triangle matrices
  u(1) = a11
  u(2)= b - a*a12/u(1)
  DO j = 3, (ns-2)
    u(j) = b - c*a/u(j-1)
  END DO
  u(ns-1)=an2 - an1*c/u(ns-2)
  ...
  y(1) = Rv(1)
  DO j = 2, (ns-1)
     y(j) = Rv(j)-y(j-1)*l(j-1)    
  END DO
  	
! Solve for f
  f(ns-1)=y(ns-1)/u(ns-1)
  DO j1 = 2, (ns-2)
      j = ns - j1
      f(j) = (y(j) - c*f(j+1))/u(j)
  END DO
...
Function Dfn(x,a,b,u,alpha,s,xi)
real(dp), intent(in) :: x,u,alpha,s,xi,a,b
real(dp)::Dfn,c,hx

  c=a*u*u -b*u
  hx = exp(-(1+xi*(x-alpha)/s)**(-1/xi))
  IF (xi >0) THEN
      if (x>(alpha -s/xi)) then 
        Dfn = a*((s/xi)**2)*(ans((1+xi*(x-alpha)/s)**(-1/xi),1-2*xi, 0)*dgamma(1-2*xi))+ &
          2*(s/xi)*(a*alpha-a*s/xi + b/2)*(ans((1+xi*(x-alpha)/s)**(-1/xi),1-xi, 0)*dgamma(1-xi))+ &
          (a*(alpha-s/xi)*(alpha-s/xi)+b*(alpha-s/xi)+c)*(1-hx)  
      end if

      if (x<(alpha -s/xi)) then
        Dfn = a*(alpha-s/xi)*(alpha-s/xi) +	&
          (s*a/xi)*((s/xi)*dgamma(1-2*xi)-2*((s/xi)-alpha)*dgamma(1-xi))+ &
          b*alpha + (s*b/xi)*(dgamma(1-xi)-1)+c
      end if
  END IF
  IF (xi <0) THEN
    if (x>(alpha -s/xi)) then 
