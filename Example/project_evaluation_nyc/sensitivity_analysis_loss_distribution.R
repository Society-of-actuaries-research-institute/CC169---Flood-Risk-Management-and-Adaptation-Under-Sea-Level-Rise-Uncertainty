...
for ( j in 1:ncol(alpha_simulated_dyke)){
  ...
  for ( i in 1:nrow(alpha_simulated_dyke)){
    ...
    if ((alpha_simulated_dyke[i,j]<=boundary_check[i]) && (check ==0)){
      ...
      dyke[i,j] =  (1+delta) * exp(12* g) *  Dfn(x = u+k2,
                                                 a = a,
                                                 b = b,
                                                 u = u,
                                                 alpha  = alpha_simulated_dyke[i,j],
...
ggsave(filename = "Loss_Distribution_dyke_NYC_lowmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_dyke_time_NYC_lowmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_Top_cover_dyke_time_NYC_lowmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_Dyke_NYC_lowmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
write.table(results, "Quantile_Dyke_NYC_lowmsl.csv", sep = "&")
...
############### PROOFING################################
# Load Boundary
boundary = read.csv(paste(opath, "threshold_nyc_proofing.csv", sep=""), sep=",", header=TRUE)
# monthly comparison
...
alpha_simualted_proofing = alpha_simulated[(12*3):(12*100),]

proofing = matrix(0,
                  ncol = ncol(alpha_simualted_proofing),
                  nrow = nrow(alpha_simualted_proofing))
...
stopping_time = matrix(0,
                       nrow = ncol(alpha_simualted_proofing))
...
for ( j in 1:ncol(alpha_simualted_proofing)){
  check = 0
  for ( i in 1:nrow(alpha_simualted_proofing)){
    
    if ((alpha_simualted_proofing[i,j]<=boundary_check[i]) && (check ==0)){
      
      proofing[i,j] =(1+delta) * exp(12* g) *  Dfn(x = u,
                                                   a = a,
                                                   b = b,
                                                   u = u,
                                                   alpha  = alpha_simualted_proofing[i,j],
                                                   s = scale,
                                                   xi = xi)
      
      top_cover[i,j] = proofing[i,j] -  (1+delta) * exp(12* g) *  Dfn(x = ustar,
                                                                      a = a,
                                                                      b = b,
                                                                      u = u,
                                                                      alpha  = alpha_simualted_proofing[i,j],
      ...
      D1 = D1a - D1b
      # For higher water levels, the damage doesn't change
      ...
      proofing[i,j] = (1+delta) * exp(12* g) * (D1+D2)
      
      # proofing[i,j] = (1+delta) * exp(12* g) * Dfn(x = u+k1,
      #                                               a = a,
      #                                              b = b1,
      #                                               u = u,
      #                                              alpha  = alpha_simualted_proofing[i,j],
      ...
      top_cover[i,j] =  proofing[i,j] -  (1+delta) * exp(12* g) * (D1+D2)
      #Dfn(x = ustar+k1,
      #a = a,
      #b = b1,
      #u = u,
...
LossesNoPolicy = unlist(mapply(function(i) LM_nopolicy(alpha = alpha_simualted_proofing[1165,i],
                                                       a = a,
                                                       b= b,
                                                       u =u),
                               1:length(alpha_simualted_proofing[1165,]),
                               SIMPLIFY = FALSE))

LossesPolicy = unlist(mapply(function(i) LM_dyke(alpha = alpha_simualted_proofing[1165,i],
                                                 a = a,
                                                 b = b,
                                                 u = u,
                                                 k = k1),
...
ggsave(filename = "Loss_Distribution_proofing_NYC.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
df = data.frame(value = c( no_policy[idx_nopolicy[1],],
                           proofing[idx_policy[1],],
                           no_policy[idx_nopolicy[2],],
                           proofing[idx_policy[2],],
                           no_policy[idx_nopolicy[3],],
                           proofing[idx_policy[3],],
                           no_policy[idx_nopolicy[4],],
                           proofing[idx_policy[4],],
                           no_policy[idx_nopolicy[5],],
                           proofing[idx_policy[5],],
                           no_policy[idx_nopolicy[6],],
                           proofing[idx_policy[6],],
                           no_policy[idx_nopolicy[7],],
                           proofing[idx_policy[7],],
                           no_policy[idx_nopolicy[8],],
                           proofing[idx_policy[8],],
                           no_policy[idx_nopolicy[9],],
                           proofing[idx_policy[9],],
                           no_policy[idx_nopolicy[10],],
                           proofing[idx_policy[10],]),
                name = factor(c( rep(10, length(no_policy[idx_nopolicy[1],]) + length(proofing[idx_policy[1],]) ),
                                 rep(20, length(no_policy[idx_nopolicy[2],]) + length(proofing[idx_policy[2],]) ),
                                 rep(30, length(no_policy[idx_nopolicy[3],]) + length(proofing[idx_policy[3],]) ),
                                 rep(40, length(no_policy[idx_nopolicy[4],]) + length(proofing[idx_policy[4],]) ),
                                 rep(50, length(no_policy[idx_nopolicy[5],]) + length(proofing[idx_policy[5],]) ),
                                 rep(60, length(no_policy[idx_nopolicy[6],]) + length(proofing[idx_policy[6],]) ),
                                 rep(70, length(no_policy[idx_nopolicy[7],]) + length(proofing[idx_policy[7],]) ),
                                 rep(80, length(no_policy[idx_nopolicy[8],]) + length(proofing[idx_policy[8],]) ),
                                 rep(90, length(no_policy[idx_nopolicy[9],]) + length(proofing[idx_policy[9],]) ),
                                 rep(100, length(no_policy[idx_nopolicy[10],]) + length(proofing[idx_policy[10],]) ))),
                Adaptation =  rep( c(rep("No Adaptation", length(no_policy[idx_nopolicy[1],])),
                                     rep("Proofing", length(proofing[idx_nopolicy[1],]))),10 )
...
ggsave(filename = "Premium_Distribution_proofing_time_NY_lowmsl.eps",
       plot = f,
       device="eps",
       width = 8,
       height = 7,
...
df = data.frame(value = c( no_policy_top_cover[idx_nopolicy[1],],
                ...
                name = factor(c( rep(10, length(no_policy_top_cover[idx_nopolicy[1],]) + length(top_cover[idx_policy[1],]) ),
                                 ...
                                 rep(100, length(no_policy_top_cover[idx_nopolicy[10],]) + length(top_cover[idx_policy[10],]) ))),
                Adaptation =  rep( c(rep("No Adaptation", length(no_policy_top_cover[idx_nopolicy[1],])),
                                     rep("Proofing", length(top_cover[idx_nopolicy[1],]))),10 )
...
ggsave(filename = "Premium_Distribution_Top_cover_Proofing_time_NYC_lowmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
df = data.frame(value = c(  no_policy[1165,],
                ...
                Adaptation = c(rep("No Adaptation",
                                   length(no_policy[1165,])),
                               rep("Proofing",
                                   length(proofing[1165,]))),
                MeanNoPolicy = mean(no_policy[1165,]),
                MeanPolicy = mean(proofing[1165,]))
...
ggsave(filename = "Premium_Distribution_proofing_NYC_lowmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Stopping_time_proofing_NYC_lowmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
results[1,] = as.numeric(quantile(no_policy[1165,]))
results[2,] = as.numeric(quantile(proofing[1165,]))

write.table(results, "Quantile_proofing_NYC_lowmsl.csv", sep = "&")
...
ggsave(filename = "Loss_Distribution_dyke_NYC_highmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_dyke_time_NYC_highmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_Top_cover_dyke_time_NYC_highmsl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_Dyke_NYC_highmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
write.table(results, "Quantile_Dyke_NYC_highmsl.csv", sep = "&")
...
ggsave(filename = "Premium_Distribution_proofing_time_NY_highmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_Top_cover_Proofing_time_NYC_highmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_proofing_NYC_highmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Stopping_time_proofing_NYC_highmsl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
write.table(results, "Quantile_proofing_NYC_highmsl.csv", sep = "&")
