...
Is <- c(I1, I2, I2, I1)
#=== Investment Analysis ===
result0 <- data.frame(matrix(NA, 3, length(sigmas)*4))
...
for(ii in 1:length(sigmas)){
  ...
  result0[,2+2*length(sigmas)+2*(ii-1)]=result[,12]
}
...
head0 <- paste("\\hline &", paste("\\multicolumn{", length(sigmas)*2-1, "}{c}{Flood-proofing then dyke}",sep=''),
               ...
               paste(
                 ...
                 paste("\\cmidrule{",length(sigmas)*2+2 ,"-",length(sigmas)*4, "}",sep='')
                 ,collapse=''),
               sep='')

idl = 2*seq(1,2*length(sigmas),1)
head1 <- paste("&", paste("$\\mu$=",sigmas, "&&",collapse='',sep=''),
               paste("$\\mu$=",sigmas[-length(sigmas)],"&&",collapse='',sep=""),
...
x.width <- xtable(result0,digit = 2, label=paste('sensitivity_portfolio_de_nyc_slr',sep=''),
                  ...
                  align =paste('ll',paste(rep('r',length(sigmas)*4-1),collapse=''),sep=''))
align(x.width) <-paste('ll',paste(rep('r',length(sigmas)*4-1),collapse=''),sep='')
comment <- list()
comment$pos <- list()
