...
# Stage 1 Boundary
boundary = read.csv(paste(opath, "threshold_cph_dike_stage1.csv", sep=""), sep=",", header=TRUE)
# monthly comparison
...
dike = matrix(0,
                   ...
                   nrow = nrow(alpha_simulated_dyie))
...
for ( j in 1:ncol(alpha_simulated_dike)){
  ...
  for ( i in 1:nrow(alpha_simulated_dike)){
      ...
      if ((alpha_simulated_dike[i,j]<=boundary_check1[i]) && (check1 == 0) ){
      ...
      } else if ( (alpha_simulated_dike[i,j]<=boundary_check2[i]) && (check2 == 0) ) {
        
        dike[i,j] = (1+delta) * exp(12* g) * Dfn(x = u +k1,
                                                 a = a,
                                                 b = b,
                                                 u = u,
                                                 alpha  = alpha_simulated_dike[i,j],
        ...
        top_cover[i,j] =  dike[i,j] - (1+delta) * exp(12* g) * Dfn(x = ustar + k1,
                                                                   a = a,
                                                                   b = b,
                                                                   u = u,
                                                                   alpha  = alpha_simulated_dike[i,j],
        ...
        dike[i,j] = (1+delta) * exp(12* g) *  Dfn(x = u+k1+k2 ,
                                                  a = a,
                                                  b = b,
                                                  u = u+k1,
                                                  alpha  = alpha_simulated_dike[i,j],
        ...
        top_cover[i,j] =  dike[i,j]  - (1+delta) * exp(12* g) *  Dfn(x = ustar+k1 +k2,
                                                                     a = a,
                                                                     b = b,
                                                                     u = u+k1,
                                                                     alpha  = alpha_simulated_dike[i,j],
...
f = f + xlab("Billion Dollars") + ylab("Premium Distribution")

#f

ggsave(filename = "Loss_Distribution_dike_CPH.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_dike_time_CPH.eps",
       plot = f,
       device="eps",
       width =7,
       height = 7,
...
df = data.frame(value = c( no_policy_top_cover[idx_nopolicy[1],]*kk -loc,
                           top_cover[idx_policy[1],],
                           no_policy_top_cover[idx_nopolicy[2],]*kk-loc,
                           top_cover[idx_policy[2],],
                           no_policy_top_cover[idx_nopolicy[3],]*kk-loc,
                           top_cover[idx_policy[3],],
                           no_policy_top_cover[idx_nopolicy[4],]*kk-loc,
                           top_cover[idx_policy[4],],
                           no_policy_top_cover[idx_nopolicy[5],]*kk-loc,
                           top_cover[idx_policy[5],],
                           no_policy_top_cover[idx_nopolicy[6],]*kk-loc,
                           top_cover[idx_policy[6],],
                           no_policy_top_cover[idx_nopolicy[7],]*kk-loc,
                           top_cover[idx_policy[7],],
                           no_policy_top_cover[idx_nopolicy[8],]*kk-loc,
                           top_cover[idx_policy[8],],
                           no_policy_top_cover[idx_nopolicy[9],]*kk-loc,
                           top_cover[idx_policy[9],],
                           no_policy_top_cover[idx_nopolicy[10],]*kk-loc,
...
ggsave(filename = "Premium_Distribution_Top_cover_dike_time_CPH.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_Dike_CPH.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Stopping_time_dyk_CPH_stage1.eps",
       plot = f,
       device="eps",
       dpi = 600)

df = data.frame(value =subset( stopping_time2,stopping_time2>0 & stopping_time2<1160))
...
ggsave(filename = "Stopping_time_dyk_CPH_stage2.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
write.table(results, "Quantile_Dike_CPH.csv", sep = "&")
