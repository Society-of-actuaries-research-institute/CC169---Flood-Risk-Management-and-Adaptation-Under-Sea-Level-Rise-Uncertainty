...
#=== Make Latex table ====
head0 <- paste("\\hline &", paste("$\\mu$=",(mus+0.15)[-length(mus)], "&&",collapse='',sep=''),
               paste("$\\mu$=",(mus+0.15)[length(mus)], sep=''),
...
library(xtable)
x.width <- xtable(result,digit = 2, label=paste('sensitivity_elevation_nyc_slr',sep=''),
...
Is <- c(I1, I2, I2, I1)
#=== Investment Analysis ===
result0 <- data.frame(matrix(NA, 3, length(mus)*4))
...
for(ii in 1:length(mus)){
  ...
  result0[,2+2*length(mus)+2*(ii-1)]=result[,12]
}
...
head0 <- paste("\\hline &", paste("\\multicolumn{", length(mus)*2-1, "}{c}{Flood-proofing then dike}",sep=''),
               ...
               paste(
                 ...
                 paste("\\cmidrule{",length(mus)*2+2 ,"-",length(mus)*4, "}",sep='')
                 ,collapse=''),
               sep='')

idl = 2*seq(1,2*length(mus),1)
head1 <- paste("&", paste("$\\mu$=",mus+0.15, "&&",collapse='',sep=''),
               paste("$\\mu$=",(mus+0.15)[-length(mus)],"&&",collapse='',sep=""),
               paste("$\\mu$=",(mus+0.15)[length(mus)], sep=''),
               "\\\\","\n",        
...
library(xtable)
x.width <- xtable(result0,digit = 2, label=paste('sensitivity_portfolio_de_nyc_slr',sep=''),
                  ...
                  align =paste('ll',paste(rep('r',length(mus)*4-1),collapse=''),sep=''))
align(x.width) <-paste('ll',paste(rep('r',length(mus)*4-1),collapse=''),sep='')
comment <- list()
comment$pos <- list()
...
print(x.width,hline.after = c(nrow(result0)),add.to.row = addtorow, 
      sanitize.text=function(x){x}, caption.placement = 'top',table.placement='H',
      floating = TRUE,include.rownames=FALSE, include.colnames=FALSE,
      file = paste(tab, "sensitivity_portfolio_de_nyc_slr", ".tex", sep=""), scalebox = 0.65)
