...
boundary = read.csv(paste(opath, "threshold_qld.csv", sep=""), sep=",", header=TRUE)
# monthly comparison
...
alpha_simulated_elevation = alpha_simulated[(12*3):(12*100),]

elevation = matrix(0,
                   ncol = ncol(alpha_simulated_elevation),
                   nrow = nrow(alpha_simulated_elevation))
...
stopping_time = matrix(0,
                       nrow = ncol(alpha_simulated_elevation))
...
## ELEVATION: Dfn(x = u+k,a = a,b = b,u = u+k,alpha  = alpha_simulated_elevation[i,j],s = scale,xi = xi)




for ( j in 1:ncol(alpha_simulated_elevation)){
  check = 0
  for ( i in 1:nrow(alpha_simulated_elevation)){
    
    if ((alpha_simulated_elevation[i,j]<=boundary_check[i]) && (check == 0)){
      
      elevation[i,j] =  (1+delta) * exp(12* g) *Dfn(x = u,
                                                    a = a,
                                                    b = b,
                                                    u = u,
                                                    alpha  = alpha_simulated_elevation[i,j],
                                                    s = scale,
                                                    xi = xi)
      
      top_cover[i,j] =  elevation[i,j] - (1+delta) * exp(12* g) *Dfn(x = ustar,
                                                                     a = a,
                                                                     b = b,
                                                                     u = u,
                                                                     alpha  = alpha_simulated_elevation[i,j],
      ...
      elevation[i,j] = (1+delta) * exp(12* g) * Dfn(x = u+k,
                                                    a = a,
                                                    b = b,
                                                    u = u+k,
                                                    alpha  = alpha_simulated_elevation[i,j],
      ...
      top_cover[i,j] = elevation[i,j] - (1+delta) * exp(12* g) * Dfn(x = ustar+k,
                                                                     a = a,
                                                                     b = b,
                                                                     u = u+k,
                                                                     alpha  = alpha_simulated_elevation[i,j],
...
LM_elevation = function(alpha, a, b,u,k){
  c=a*u*u -b*u
  L =max( a * (alpha- k )^2 + b*(alpha-k) + c,0)
  return(L)
}

LossesNoPolicy = unlist(mapply(function(i) LM_nopolicy(alpha = alpha_simulated_elevation[1165,i],
                                                       a = a,
                                                       b= b,
                                                       u =u),
                               1:length(alpha_simulated_elevation[1165,]),
                               SIMPLIFY = FALSE))

LossesPolicy = unlist(mapply(function(i) LM_elevation(alpha = alpha_simulated_elevation[1165,i],
                                                      a = a,
                                                      b = b,
                                                      u = u,
                                                      k = k),
...
ggsave(filename = "Loss_Distribution_elevation_QLD_lmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
df = data.frame(value = c( no_policy[idx_nopolicy[1],],
                           elevation[idx_policy[1],],
                           no_policy[idx_nopolicy[2],],
                           elevation[idx_policy[2],],
                           no_policy[idx_nopolicy[3],],
                           elevation[idx_policy[3],],
                           no_policy[idx_nopolicy[4],],
                           elevation[idx_policy[4],],
                           no_policy[idx_nopolicy[5],],
                           elevation[idx_policy[5],],
                           no_policy[idx_nopolicy[6],],
                           elevation[idx_policy[6],],
                           no_policy[idx_nopolicy[7],],
                           elevation[idx_policy[7],],
                           no_policy[idx_nopolicy[8],],
                           elevation[idx_policy[8],],
                           no_policy[idx_nopolicy[9],],
                           elevation[idx_policy[9],],
                           no_policy[idx_nopolicy[10],],
                           elevation[idx_policy[10],]),
                name = factor(c( rep(10, length(no_policy[idx_nopolicy[1],]) + length(elevation[idx_policy[1],]) ),
                                 rep(20, length(no_policy[idx_nopolicy[2],]) + length(elevation[idx_policy[2],]) ),
                                 rep(30, length(no_policy[idx_nopolicy[3],]) + length(elevation[idx_policy[3],]) ),
                                 rep(40, length(no_policy[idx_nopolicy[4],]) + length(elevation[idx_policy[4],]) ),
                                 rep(50, length(no_policy[idx_nopolicy[5],]) + length(elevation[idx_policy[5],]) ),
                                 rep(60, length(no_policy[idx_nopolicy[6],]) + length(elevation[idx_policy[6],]) ),
                                 rep(70, length(no_policy[idx_nopolicy[7],]) + length(elevation[idx_policy[7],]) ),
                                 rep(80, length(no_policy[idx_nopolicy[8],]) + length(elevation[idx_policy[8],]) ),
                                 rep(90, length(no_policy[idx_nopolicy[9],]) + length(elevation[idx_policy[9],]) ),
                                 rep(100, length(no_policy[idx_nopolicy[10],]) + length(elevation[idx_policy[10],]) ))),
                Adaptation =  rep( c(rep("No Adaptation", length(no_policy[idx_nopolicy[1],])),
                                     rep("Elevation", length(elevation[idx_nopolicy[1],]))),10 )
...
f = f + xlab("Years") + ylab("Premium Distribution ($Billion) on log-scale")
...
ggsave(filename = "Premium_Distribution_elevation_logscale_time_QLD_lmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
f = f + xlab("Years") + ylab("Premium Distribution ($Million)")
...
ggsave(filename = "Premium_Distribution_elevation_time_QLD_lmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
df = data.frame(value = c( no_policy_top_cover[idx_nopolicy[1],],
                ...
                Adaptation =  rep( c(rep("No Adaptation", length(no_policy_top_cover[idx_nopolicy[1],])),
                                     rep("Elevation", length(top_cover[idx_nopolicy[1],]))),10 )
...
ggsave(filename = "Premium_Distribution_Top_cover_elevation_time_QLD_lmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_Top_cover_elevation_logscale_time_QLD_lmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
df = data.frame(value = c(  no_policy[1165,],
                ...
                Adaptation = c(rep("No Adaptation",
                               ...
                               rep("Elevation",
                                   length(elevation[1165,]))),
                MeanNoPolicy = mean(no_policy[1165,]),
                MeanPolicy = mean(elevation[1165,]))

f = ggplot(df, aes( x = value,
                    color = Adaptation,
                    fill = Adaptation)) 

f = f + geom_histogram(binwidth=0.003,
                       alpha=0.5,
                       position="identity")
...
ggsave(filename = "Premium_Distribution_elevation_QLD_lmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Stopping_time_elevation_qld_lmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
results[2,] = as.numeric(quantile(elevation[1165,]))

write.table(results, "Quantile_Elevation_lmslvl.csv", sep = "&")
...
ggsave(filename = "Loss_Distribution_elevation_QLD_highmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_elevation_logscale_time_QLD_highmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_elevation_time_QLD_highmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_Top_cover_elevation_time_QLD_highmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Premium_Distribution_Top_cover_elevation_logscale_time_QLD_highmslvl.eps",
       plot = f,
       width = 7,
       height = 7,
       device="eps",
...
ggsave(filename = "Premium_Distribution_elevation_QLD_highmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
ggsave(filename = "Stopping_time_elevation_qld_highmslvl.eps",
       plot = f,
       device="eps",
       dpi = 600)
...
write.table(results, "Quantile_Elevation_highmslvl.csv", sep = "&")
